...
const DEFAULTS = {
  ...
  // And this one is not about abuse so much as shape. A modem call cannot go
  // faster than its carrier; bypass has no carrier, so without a cap a board
  // and a browser are wired together at whatever the two TCP connections
  // manage. 128000 is twice the fastest protocol here (V.90's 56000), which
  // leaves bypass comfortably the quickest way to reach a board while keeping
  // an ANSI "movie" or a file send from costing what a hundred of them would.
  // Both directions, and NEVER announced — nothing in the UI says a call is
  // paced. 0 disables. → lib/throttle.js.
  directMaxBitsPerSecond: 128000,
  ...
  // High-traffic mode: at this many live sessions, a modem call is SIMULATED —
  // the server runs no DSP, the browser plays the handshake locally, and payload
  // rides the socket paced at the protocol's own bit rate. The per-call CPU of
  // an answer-side modem is what this removes. 0 disables, 1 simulates every
  // call, N takes effect at N sessions or more.
  simulateModemAtSessions: 0,
  ...
  // Optional Google sync. Empty = the feature does not exist: no sign-in
  // control is served, nothing is fetched from Google, and every visitor keeps
  // their favourites in their own browser exactly as before. Absent means off,
  // like the four sysop keys and for the same reason — configload is strict, so
  // a required new key would stop every deployment that has not been edited.
  //
  // This is the ONLY server-side trace of the feature. The data lives in the
  // user's own Google Drive and this server neither sees nor stores any of it.
  googleClientId: '',
  sysopEnabled: false,
  sysopUser: '',
  ...
  // How long a credential the server has already verified is trusted before the
  // password hash is run again. It is a CPU memo and not a session: an expired
  // one costs one scrypt, and the browser is never told anything. A week is
  // chosen because the page it guards is read-only and the credential is sent on
  // every poll anyway — nothing is kept alive here that the operator's browser
  // was not already re-presenting. Bound to the configured hash, so changing
  // the password invalidates every memo on the spot. → lib/sysop.js.
  sysopSessionHours: 168,
};
...
const RULES = {
  ...
  directMaxBitsPerSecond:   { type: 'int',    min: 0, max: 100000000 },
  ...
  simulateModemAtSessions:  { type: 'int',    min: 0, max: 10000 },
  ...
  // A public value, not a secret — it is substituted into a served page. The
  // flow used here has no client secret at all; anything pasted in that looks
  // like one is a mistake worth catching at boot.
  googleClientId: {
    type: 'string',
    check: (v) => (v === '' || /^[0-9][0-9-]*[A-Za-z0-9_-]*\.apps\.googleusercontent\.com$/.test(v))
      ? null
      : 'googleClientId must be the OAuth client ID from the Google Cloud console — ' +
        'it ends in .apps.googleusercontent.com. It is not a client secret, and this ' +
        'server never needs one.',
  },
  sysopEnabled:             { type: 'bool' },
  sysopUser:                { type: 'string' },
  sysopRefreshSeconds:      { type: 'int',    min: 1, max: 3600 },
  sysopSessionHours:        { type: 'number', min: 0, max: 8760 },
...
function tokens() {
  ...
  return {
    ...
    // Empty when unconfigured, which is what main.js tests to decide whether to
