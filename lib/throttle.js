...
// is paced by physics on the way OUT — the DSP hands the transport bytes at the
// protocol's own rate and nothing can go faster than the carrier — but nothing
// paces what goes IN, which is what server.js's modemFlow() is for. Read that
// distinction before extending either: the difference between the two is where
// an 11 MB transmit queue once accumulated. Bypass has no carrier, so a
