 ...
 *                                APPEND-ONLY: each monthly edition is merged
 *                                into the cache, never substituted for it, and
 *                                every entry carries the date we first saw it.
 *                                → mergeEntries().
 *
...
// ── append-only merge ───────────────────────────────────────────────────────
// The cached guide is a RECORD OF EVERY BOARD WE HAVE EVER SEEN, not a copy of
// this month's file. A monthly edition is merged into it: entries new to us are
// appended with the date we first saw them, entries already known keep the date
// they were first seen, and an entry that has left the guide is NEVER dropped —
// the blacklist is the only thing that removes a board from what we offer.
//
// Two reasons it works this way. `added` is ours: the guide publishes no
// first-listed date, so "newest" can only mean newest to this instance, and
// that meaning is only stable if the record is never rebuilt. And a board that
// falls out of one edition (a sysop's DNS lapses for a month) would otherwise
// take its dial count's context and any favourite's listing with it.
//
// Identity is name + host:port, matching what an entry IS rather than where it
// answers: the current edition has ten pairs sharing an address as genuinely
// distinct listings (see csvToEntries), so host:port alone would collapse them.
// The cost is that a RENAMED board arrives as a second entry beside its old
// one; that is the trade this key makes deliberately, and the blacklist or a
// hand edit of cache/guide.json is the answer if one ever matters.
const entryKey = (e) =>
  `${String(e.host).toLowerCase()}:${parseInt(e.port, 10) || 23}|${String(e.name).toLowerCase()}`;

/** Merge a freshly-parsed edition into what we already have. Never deletes. */
function mergeEntries(existing, fresh, when) {
  const day = String(when || new Date().toISOString()).slice(0, 10);
  const out = [];
  const seen = new Map();
  for (const e of existing || []) {
    if (!e || !e.host || !e.name) continue;
    // A record written before `added` existed is backfilled with this run's
    // date rather than left blank: an entry with no date would sort as either
    // oldest or newest depending on the comparator, and both are wrong. They
    // are all equally old, which is exactly what one shared date says.
    const rec = { name: e.name, host: e.host, port: e.port, added: e.added || day };
    seen.set(entryKey(rec), rec);
    out.push(rec);
  }
  let added = 0;
  for (const e of fresh) {
    ...
    const rec = { name: e.name, host: e.host, port: e.port, added: day };
    seen.set(k, rec);
    out.push(rec);
    added++;
  }
...
/** What's cached now, for merging into. Read from disk so an ingest is safe
 *  before start() has loaded anything. */
function cachedEntries() {
  if (_guide && Array.isArray(_guide.entries)) return _guide.entries;
  const g = readJSON(GUIDE_JSON, null);
  return g && Array.isArray(g.entries) ? g.entries : [];
}
...
/**
 * The name we will store a downloaded archive under.
 *
 * This is a filename chosen by a REMOTE PAGE — the href of a link on
 * telnetbbsguide.com — and it used to be `decodeURIComponent(...)` joined
 * straight onto cache/. A link ending `..%2f..%2f..%2fpublic%2fmain.js` then
 * wrote whatever was downloaded over a served file. path.basename() first,
 * because the decode is what can reintroduce a separator, and then a shape
 * check: this is a name we invent a path from, not text we display, so anything
 * that is not obviously one of these archives is refused rather than repaired.
 */
function safeArchiveName(zipUrl) {
  let raw = String(zipUrl).split('?')[0].split('#')[0].split('/').pop() || '';
  try { raw = decodeURIComponent(raw); } catch (_) { /* keep it undecoded */ }
  const name = path.basename(raw);
  if (!/^[A-Za-z0-9._-]+\.zip$/i.test(name) || name.startsWith('.')) {
    throw new Error(`refusing an archive name from the listing page: ${JSON.stringify(raw)}`);
  }
  return name;
}
...
async function refresh({ force = false, log = () => {} } = {}) {
  ...
  const file = safeArchiveName(zipUrl);
  ...
  const fresh = csvToEntries(csv);
  if (fresh.length < MIN_ROWS) {
    throw new Error(`only ${fresh.length} rows parsed — refusing to replace the cache`);
  }

  // 4. Commit: keep the artefacts for inspection, then merge the edition in.
  const fetched = new Date().toISOString();
  const { entries, added } = mergeEntries(cachedEntries(), fresh, fetched);
  fs.writeFileSync(path.join(CACHE_DIR, file), zip.body);
  fs.writeFileSync(path.join(CACHE_DIR, 'bbslist.csv'), csv);
  writeJSONAtomic(GUIDE_JSON, { source: LIST_PAGE, file, fetched, entries });
  meta.file = file;
  ...
  log(`updated: ${added} new from ${file}; ${entries.length} known in total`);
  return { changed: true, reason: 'updated', count: entries.length, added, file };
}
...
function ingestZipFile(zipPath, { log = () => {} } = {}) {
  ...
  fs.writeFileSync(path.join(CACHE_DIR, 'bbslist.csv'), csv);
  // Merged, exactly as a fetched edition is: a hand-dropped zip is the same
  // data by a different route and must not reset anyone's first-seen dates.
  const fetched = new Date().toISOString();
  const { entries, added } = mergeEntries(cachedEntries(), fresh, fetched);
  writeJSONAtomic(GUIDE_JSON, { source: 'manual', file, fetched, entries });
  const meta = readJSON(META_JSON, {});
  meta.file = file; meta.count = entries.length; meta.manual = true;
  meta.zipETag = null; meta.zipModified = null;
  writeJSONAtomic(META_JSON, meta);
  loadGuide();
  log(`ingested ${file}: ${added} new; ${entries.length} known in total`);
  return { changed: true, reason: 'manual', count: entries.length, added, file };
}
...
module.exports = { start, refresh, directory, loadGuide, loadCurated, safeArchiveName,
                   ingestZipFile, ingestDropped, mergeEntries, entryKey,
