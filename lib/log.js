...
// ── surviving a restart ─────────────────────────────────────────────────────
// The counters above are today's, and a restart used to lose them: the sysop
// page's whole "today" block went back to zero and the end-of-day summary
// reported only the hours since the last start. They are mirrored to one JSON
// file for that and nothing else — debounced and written temp-then-rename, the
// way cache/bbsStats.json is.
//
// It sits in the LOG directory rather than cache/ because these are the daily
// summary's own counters: it follows the operator's `dir` setting, so a harness
// that writes a scratch logging.json is isolated from the operator's numbers by
// the same mechanism that already isolates its log lines. prune() only ever
// deletes a stamped KIND-YYYY-MM-DD.log, so this file is not swept up by it.
//
// It is deliberately NOT a store. Nothing reads this file except a restore at
// boot, the restore is refused unless the stamp inside it is TODAY, and anything
// missing, unparseable or from yesterday simply leaves the counters fresh —
// which is exactly the behaviour that predates the file. A summary block is
// still written from memory and is still the record; this only decides what is
// in memory to write.
//
// Off until restoreCounters() is called, so a harness that never asks for it
// neither reads nor writes anything.

const COUNTER_NAME = 'dayCounters.json';
const COUNTER_FLUSH_MS = 10000;

/** Resolved per call: logDir() depends on config, which is reloadable. */
function counterFile() { return path.join(logDir(), COUNTER_NAME); }

let _persist = false;
let _cDirty = false;
let _cTimer = null;

function countersToJSON(c = counters) {
  // uniqueIps is written out in full: the day's count is only meaningful if the
  // addresses themselves come back, since a restored count plus fresh arrivals
  // would double-count everyone who returns. They are already in the access log,
  // so this exposes nothing new — but it is a second file holding them, and it
  // is capped by MAX_UNIQUE_IPS like the set itself.
  return { ...snapshot(c), uniqueIps: [...c.uniqueIps] };
}

function countersFromJSON(raw) {
  const c = freshCounters();
  if (!raw || typeof raw !== 'object') return c;
  const num = (v) => (typeof v === 'number' && isFinite(v) && v >= 0 ? v : 0);
  const obj = (v) => (v && typeof v === 'object' && !Array.isArray(v) ? { ...v } : {});
  c.day = String(raw.day || c.day);
  c.started = String(raw.started || c.started);
  for (const k of ['httpRequests', 'httpBytes', 'sessions', 'dials', 'connects',
                   'failures', 'telnetIn', 'telnetOut', 'audioIn', 'audioOut',
                   'sessionSeconds']) c[k] = num(raw[k]);
  c.httpStatus = obj(raw.httpStatus);
  c.failuresByCode = obj(raw.failuresByCode);
  c.boards = obj(raw.boards);
  if (Array.isArray(raw.uniqueIps)) {
    for (const ip of raw.uniqueIps.slice(0, MAX_UNIQUE_IPS)) c.uniqueIps.add(String(ip));
  }
  return c;
}

function flushCounters() {
  if (!_persist || !_cDirty) return;
  _cDirty = false;
  if (!ensureDir()) return;
  try {
    const file = counterFile();
    const tmp = file + '.tmp';
    fs.writeFileSync(tmp, JSON.stringify(countersToJSON()));
    fs.renameSync(tmp, file);
  } catch (_) { /* today's counters are not worth taking the server down for */ }
}

/** Mark the counters changed. Called by everything that changes one. */
function touch() {
  if (!_persist) return;
  _cDirty = true;
  if (_cTimer) return;
  _cTimer = setTimeout(() => { _cTimer = null; flushCounters(); }, COUNTER_FLUSH_MS);
  if (_cTimer.unref) _cTimer.unref();   // never hold the process open
}

/**
 * Turn the mirror on and adopt what it holds, if it is from today. → server.js,
 * at boot. Returns the stamp restored, or null.
 */
function restoreCounters() {
  _persist = true;
  let raw;
  try { raw = JSON.parse(fs.readFileSync(counterFile(), 'utf8')); }
  catch (_) { return null; }
  // A stamp that is not today is yesterday's day, and yesterday's summary has
  // either been written or has been missed; either way it is not today's total.
  if (!raw || raw.day !== dayStamp()) return null;
  counters = countersFromJSON(raw);
  return counters.day;
}
...
/**
 * Today's counters, as plain JSON-able values. → lib/sysop.js.
 *
 * A copy, not the live object: the counters are mutated on every request and
 * every call, and handing a reader the real one would let a caller hold a
 * reference across the midnight rollover (writeSummary swaps in a fresh set) and
 * quietly report yesterday for ever. `uniqueIps` is a Set and becomes its size —
 * the addresses themselves are nobody's business but the access log's.
 *
 * These reset at the daily summary, so "today" here means the server's local
 * calendar day and not a rolling 24 hours.
 */
function snapshot(c = counters) {
  return {
    day: c.day,
    started: c.started,
    httpRequests: c.httpRequests,
    httpStatus: { ...c.httpStatus },
    httpBytes: c.httpBytes,
...
function writeSummary(endedAt = new Date(Date.now() - 60000)) {
  ...
  // Overwrite the mirror with the empty set immediately rather than on the
  // debounce: a restart in the seconds after midnight would otherwise find the
  // ended day's file still on disk, and only its stamp would stop it being
  // adopted as today.
  _cDirty = true; flushCounters();
...
function hookExit() {
  ...
  process.on('exit', () => { flushCounters(); for (const k of Object.keys(streams)) { try { fs.closeSync(streams[k].fd); } catch (_) {} } });
}
...
module.exports = {
  ...
  prune, writeSummary, summaryText, snapshot, startDailySummary, stopDailySummary,
  restoreCounters, flushCounters, close, hookExit,
  // exposed for tools/tests/logtest.js
  _internals: { dayStamp, clfTime, msUntilMidnight, freshCounters, human, sanitize,
                counterFile, countersToJSON, countersFromJSON, MAX_UNIQUE_IPS,
