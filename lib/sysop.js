 ...
 *
 * Requests carrying the SAME credential JOIN the verification in flight rather
 * than being refused by it, and that is not a softening of the rule above — it
 * is still one scrypt per distinct credential. Refusing them instead answered
 * 401 to a credential that was about to verify, which a browser reads as the
 * password being wrong and re-prompts for: two tabs polling this page collide
 * on every memo expiry. Only a DIFFERENT credential arriving mid-flight is
 * refused, which is the case the bound exists for.
 *
 * The memo is keyed on the header AND the configured hash, so changing the
 * password invalidates every memo on the spot rather than leaving an old
 * credential good until its entry expires.
 *
...
// How long a verified Authorization header is trusted without re-hashing, when
// config/site.json says nothing. Absent means the default, which is the property
// every sysop* setting has to keep — see the header block in lib/site.js.
const MEMO_MS = 168 * 60 * 60 * 1000;

/** The memo lifetime in force. 0 (or a missing hash) re-hashes every request. */
function memoMs(cfg) {
  const h = cfg && cfg.sysopSessionHours;
  return (typeof h === 'number' && h >= 0) ? h * 3600000 : MEMO_MS;
}
...
const _memo = new Map();          // memo key -> expiry ms
const _waiting = new Map();       // memo key -> requests joined to one scrypt
...
function guard(req, res, cfg, next) {
  ...
  // The hash is part of the key, so a password change strands every existing
  // memo rather than honouring it to its expiry.
  const key = header + '\n' + String(cfg.sysopPasswordHash || '');

  const memoUntil = _memo.get(key);
  ...
  // Same credential already being verified: wait for that answer instead of
  // being refused by it. Still one scrypt — see the header block.
  const joined = _waiting.get(key);
  if (joined) { joined.push({ res, userOk, next }); return; }

  if (_verifying) return setTimeout(() => unauthorized(res, cfg.brand), FAIL_DELAY_MS);

  _verifying = true;
  _waiting.set(key, [{ res, userOk, next }]);
  verifyPassword(pass, cfg.sysopPasswordHash, (passOk) => {
    _verifying = false;
    const waiters = _waiting.get(key) || [];
    _waiting.delete(key);
    // Every waiter presented the same header, so `userOk` is the same answer for
    // all of them; it is carried per request only because it is computed there.
    if (userOk && passOk) _memo.set(key, Date.now() + memoMs(cfg));
    for (const w of waiters) {
      if (userOk && passOk) w.next();
      else setTimeout(() => unauthorized(w.res, cfg.brand), FAIL_DELAY_MS);
    }
  });
}
 ...
 * @param {object[]} [src.recent]  ended sessions, newest first
...
function snapshot(src) {
  ...
  // Ended sessions, newest first, as server.js recorded them at teardown.
  const recent = [];
  for (const r of (src.recent || [])) {
    const d = src.describe(r.host, r.port);
    const end = r.endedAt || now;
    recent.push({
      id: r.id, ip: r.ip, name: d.name || '', tier: d.tier || '',
      host: r.host, port: r.port, proto: r.proto || '', bps: r.bps || 0,
      direct: !!r.direct,
      simulated: !!r.simulated,
      started: new Date(r.openedAt).toISOString(),
      ended: new Date(end).toISOString(),
      totalSec: Math.round((end - r.openedAt) / 1000),
      // Time on the board, as linkSec is for a live call; 0 = never had carrier.
      linkSec: r.linkAt ? Math.round((end - r.linkAt) / 1000) : 0,
      carrier: !!r.linkAt,
      reason: r.reason || '',
      failCode: r.failCode || '',
      bytes: r.bytes ? { telnetIn: r.bytes.telnetIn, telnetOut: r.bytes.telnetOut } : null,
    });
  }
  ...
  return {
    ...
    limits: {
      ...
      simulateModemAtSessions: src.cfg.simulateModemAtSessions,
...
module.exports = { enabled, guard, snapshot, page, hashPassword, verifyPassword,
                   ...
                   _internals: { MEMO_MS, FAIL_DELAY_MS, SCRYPT, memoMs } };
