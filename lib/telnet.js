...
// The browser's grid is 25 rows of CP437 and either 80 or 40 columns — 40 when
// the 9×14 font is active, which is the only way that mode is reached (see
// public/fonts/index.js). The width is therefore no longer a constant: it
// arrives with the dial message and is applied with setWindow() before the BBS
// socket is opened. These remain the fallback for a caller that says nothing.
...
class TelnetFilter {
  ...
  /**
   * Set the window this filter speaks for. Call BEFORE the BBS socket is opened
   * — that is the only moment it can matter, because NAWS goes out during
   * negotiation and there is no way to revise it afterwards: once a carrier is
   * up nothing but modulated audio crosses the browser socket, so the server
   * cannot learn of a resize mid-call. A terminal that changes width during a
   * call is simply out of step with the BBS until the next dial.
   *
   * Values are clamped to what NAWS can express and to something a terminal
   * could plausibly be, so a malformed dial message cannot put nonsense on the
   * wire (or a zero width, which some BBSes read as "unknown" and others as a
   * reason to hang up).
   *
   * @param {number} cols
   * @param {number} rows
   * @returns {boolean} false if the size was rejected or NAWS had already gone
   */
  setWindow(cols, rows) {
    if (this._nawsSent) return false;      // too late; the BBS has been told
    const c = Math.round(Number(cols)), r = Math.round(Number(rows));
    if (!Number.isFinite(c) || !Number.isFinite(r)) return false;
    if (c < 1 || c > 65535 || r < 1 || r > 65535) return false;
  ...
  _sendNAWS() {
    // Sent once and never updated: the size is fixed for the duration of a call
    // by construction (see setWindow), so there are no resize events to report.
    if (this._nawsSent) return;
    this._nawsSent = true;
    const reply = [IAC, SB, OPT_NAWS];
    // 16-bit big-endian width then height. Neither 80/40 nor 25 hits the
    // escaping rule, but the size is caller-supplied now, so escape regardless.
...
/**
 * Double every literal 0xFF in a PAYLOAD stream bound for the peer.
 *
 * The inverse of what process() does on the way in, and the two belong
 * together: a peer that sends us IAC IAC means one 0xFF byte, and a 0xFF byte
 * we send means IAC unless we say otherwise. Without this a binary upload —
 * an X/Y/ZMODEM block, which is full of 0xFF — reads to the BBS as a stream of
 * telnet commands and desynchronises its parser.
 *
 * PAYLOAD ONLY. Negotiation replies are built by _send()/pushEscaped(), which
 * have already escaped their own subnegotiation payloads and whose command
 * bytes are meant to be read as commands; passing one through here would
 * double the IACs that make them commands at all. That is why this is a free
 * function rather than something the filter applies on the way out — the
 * filter's own output must not touch it. See server.js's toBBSPayload().
 *
 * Returns the input unchanged when there is nothing to escape, which is every
 * keystroke and every byte of ANSI a board has ever sent, so the common path
 * costs one scan and no copy.
 *
 * @param {Uint8Array|Buffer} bytes
 * @returns {Uint8Array|Buffer} same type as the input
 */
function escapeIAC(bytes) {
  let n = 0;
  for (let i = 0; i < bytes.length; i++) if (bytes[i] === IAC) n++;
  if (n === 0) return bytes;
  // Buffer in, Buffer out: server.js writes the result straight to a socket,
  // and a Uint8Array there would be a needless conversion on the one path that
  // is by definition carrying bulk data.
  const out = (typeof Buffer !== 'undefined' && Buffer.isBuffer(bytes))
    ? Buffer.allocUnsafe(bytes.length + n)
    : new Uint8Array(bytes.length + n);
  let j = 0;
  for (let i = 0; i < bytes.length; i++) {
    out[j++] = bytes[i];
    if (bytes[i] === IAC) out[j++] = IAC;
  }
  return out;
}
