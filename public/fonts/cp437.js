/*
 * fonts/cp437.js - CP437 (Code Page 437) to Unicode, all 256 positions.
 *
 * Needed only by the OUTLINE (TTF) font path: a bitmap font is indexed by byte
 * value directly, but fillText needs the character the byte MEANS. See FONTS.md
 * section 2.4.
 *
 * GENERATED MECHANICALLY, not hand-typed - it is exactly what Python's `cp437`
 * codec produces:
 *
 *     [ord(bytes([i]).decode('cp437')) for i in range(256)]
 *
 * Hand-typing 256 codepoints is the kind of task that produces a table which is
 * 254 entries correct, and the two wrong ones are box-drawing characters nobody
 * looks at closely. tools/tests/ttftest.js re-derives the mapping independently and
 * compares, so a transcription slip fails a test rather than shipping.
 *
 * Two positions are policy rather than lookup. 0x00 maps to U+0000 (NUL) and
 * 0xFF to U+00A0 (NBSP); both must render BLANK rather than as .notdef boxes,
 * which is handled at the atlas builder rather than by editing this table -
 * the mapping itself stays a faithful CP437 transcription.
 *
 * All 256 codepoints are distinct, and all 256 resolve to real glyphs in
 * Flexi_IBM_VGA_False_437 (288 cmap entries), so nothing falls through.
 *
 * SynthLink's own code, GPL-3.0-or-later.
 */

/** CP437 byte -> Unicode codepoint. Index is the byte, value is the codepoint. */
export const CP437_TO_UNICODE = new Uint16Array([
...
/**
 * The CP437 byte as a JS string, ready for fillText.
 * Cached: building 256 single-character strings once beats doing it per atlas.
 */
export const CP437_CHARS = Array.from(CP437_TO_UNICODE, (c) => String.fromCharCode(c));
