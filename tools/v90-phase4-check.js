 ...
 *
 * Run: node tools/v90-phase4-check.js
 */
...
for (let t = 0; t < 400; t++) {
  const c = cp.slice();
  const p = 18 + Math.floor(Math.random() * (272 - 18));
  c[p] ^= 1;
  if (!P.parseCP(c, 1).crcOk) caught++;
}
console.log(`CP CRC: ${caught}/400 single-bit corruptions detected`);
ok(caught === 400, 'every single-bit corruption is caught by the CRC');
