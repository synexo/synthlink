...
// ── 1. config ───────────────────────────────────────────────────────────────
{
  ...
  eq(typeof c.connectTimeoutMs, 'number', 'config: connectTimeoutMs is numeric');
}
...
// ── 3. Apache combined format ───────────────────────────────────────────────
{
  ...
  const re = /^(\S+) (\S+) (\S+) \[(\d{2}\/[A-Z][a-z]{2}\/\d{4}:\d{2}:\d{2}:\d{2} [+-]\d{4})\] "([^"]*)" (\d{3}) (\d+|-) "([^"]*)" "([^"]*)"$/;
  const m = re.exec(first);
  ok(m, 'combined: line matches the Apache combined grammar');
  if (m) {
    eq(m[1], '198.51.100.4', 'combined: host field');
    eq(m[2], '-', 'combined: ident is -');
    eq(m[3], '-', 'combined: authuser is -');
    eq(m[5], 'GET /bbs.json?v=2 HTTP/1.1', 'combined: request line keeps the query string');
    eq(m[6], '200', 'combined: status');
    eq(m[7], '17233', 'combined: byte count');
    eq(m[8], 'https://example.org/x', 'combined: referer');
    eq(m[9], 'Mozilla/5.0 (X)', 'combined: user-agent');
  }
...
// ── 4. session / dial / connect / fail / end events ─────────────────────────
{
  ...
  ok(/SESSION id=7 open ua="Mozilla\/5\.0"/.test(access), 'event: SESSION open carries the UA');
  ok(/DIAL\s+id=7 bbs\.example\.org:23 via V32bis/.test(access), 'event: DIAL names the protocol');
  ok(/CONNECT id=7 bbs\.example\.org:23 carrier V32bis @ 14400 bps/.test(access), 'event: CONNECT');
  ok(/END\s+id=7 bbs\.example\.org:23 dur=346\.1s/.test(access), 'event: END carries the duration');
  ok(/telnetIn=48210 telnetOut=1204 audioIn=5120000 audioOut=5140000/.test(access),
     'event: END carries all four byte totals');
  ok(/FAIL\s+id=8 dead\.example\.org:2323 ECONNREFUSED/.test(access), 'event: FAIL appears in the access log');
  ...
  ok(/dead\.example\.org:2323 ECONNREFUSED/.test(failLog), 'failLog: address and error code');
  ok(/name="Dead Board"/.test(failLog), 'failLog: board name, for review');
  ok(/tier=guide/.test(failLog), 'failLog: which tier it came from');
  ok(/client=198\.51\.100\.4/.test(failLog), 'failLog: who was trying');
...
// the console must not be written at all unless asked for.
{
  ...
  fs.writeFileSync(CFG, JSON.stringify({ ...log.config(), debug: true }));
  log.loadConfig();
  log.debug('198.51.100.4', 7, 'telnet→modem 12B');
  log.close();
  const on = fs.readFileSync(path.join(LOGDIR, `accessLog-${dayStamp()}.log`), 'utf8');
  ok(/DEBUG\s+id=7 telnet→modem 12B/.test(on), 'debug: written when debug is on');
  fs.writeFileSync(CFG, JSON.stringify({ ...log.config(), debug: false }));
  log.loadConfig();
}
...
// ── 8. daily summary ────────────────────────────────────────────────────────
{
  ...
  ok(/HTTP requests\s+2/.test(text), 'summary: HTTP request count');
  ok(/200=1/.test(text) && /404=1/.test(text), 'summary: status breakdown');
  // Two HTTP clients plus the session's own address.
  ok(/unique clients\s+3/.test(text), 'summary: unique clients');
  ok(/BBS dials\s+1/.test(text), 'summary: dial count');
  ok(/BBS connects\s+3/.test(text), 'summary: connect count');
  ok(/BBS failures\s+1/.test(text) && /ETIMEDOUT=1/.test(text), 'summary: failures with codes');
  ok(/distinct boards\s+2/.test(text), 'summary: distinct boards');
  ok(/2\.0 KB/.test(text), 'summary: byte totals are human-readable');
  ok(/a\.example\.org:23/.test(text), 'summary: top boards listed');
